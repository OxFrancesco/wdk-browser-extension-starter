...
| Multiple custom EVM chains | `wallet_addEthereumChain` handling in `entrypoints/background.ts`; encrypted `customEvmChains` vault storage; dynamic WDK registration in `src/lib/wdk-adapter.ts`; popup selection/removal in `entrypoints/popup/App.tsx` | Implemented for approved EIP-155 mainnet/testnet networks |
| Website dApp connection | `public/ethereum-provider.js` injects `window.ethereum`; `entrypoints/content.ts` bridges EIP-1193 requests; `entrypoints/background.ts` enforces per-origin approval; browser E2E approves `eth_requestAccounts`, adds a custom EIP-155 network, and switches to it from an HTTPS page | Implemented for EVM dApps |
