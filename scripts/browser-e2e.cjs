...
async function main() {
  ...
  try {
    ...
    await page.locator('label[for="network-mode"]').getByText('Testnet', { exact: true }).waitFor({ timeout: 15_000 });

    await page.getByRole('button', { name: 'Add account' }).click();
    await selectByName(page, 2, 'Account 2');
    await page.getByText(/0x6Fac4D/i).waitFor({ timeout: 90_000 });
    ...
    const customChainId = await dappPage.evaluate(() => window.ethereum.request({ method: 'eth_chainId' }));
    if (customChainId !== '0x2105') {
      throw new Error(`Expected custom Base chain id 0x2105, got ${customChainId}.`);
    }
